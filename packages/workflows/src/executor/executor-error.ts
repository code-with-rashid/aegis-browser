 ...
 * recording). `TOOL_CALL_FAILED` means the tool itself ran and reported failure.
 ...
 * simply being unmet, which is a `false` result, not an error. `HEAL_FAILED` (#113) means
 * a self-heal attempt on a failed step didn't produce a working fix — the Navigator
 * couldn't find one, its proposed tool call itself failed, or the fix still didn't
 * satisfy the step's `expect`.
 */
export type WorkflowExecutionErrorCode =
  'TARGET_NOT_FOUND' | 'TOOL_CALL_FAILED' | 'POST_CONDITION_CHECK_FAILED' | 'HEAL_FAILED';
